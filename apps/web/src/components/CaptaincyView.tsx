    ...
    <div className="p-6 space-y-8 max-w-7xl mx-auto">
      ...
      <div>
        <h2 className="text-2xl font-extrabold text-slate-100 flex items-center gap-2">
          ...
          <span>Captaincy shortlist {captainData?.gameweek ? `— GW${captainData.gameweek}` : ""}</span>
        </h2>
        <p className="text-xs text-slate-400 mt-1">
          Eligible outfield starters ranked by expected points, minutes security and upside.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-surface border border-emerald-500/30 rounded-3xl p-6 relative overflow-hidden shadow-xl shadow-emerald-500/5 space-y-4">
          ...
          <div>
            <h3 className="text-2xl font-black text-slate-100">{shield?.web_name ?? "No eligible pick"}</h3>
          ...
          <div className="bg-surfaceHover/60 border border-surfaceBorder rounded-2xl p-4 flex items-baseline justify-between font-mono">
            <div>
              <div className="text-2xl font-extrabold text-emerald-400">{shield?.mean_xp ?? "—"} xP</div>
            ...
            <div className="text-right text-xs text-slate-300 font-sans">
              <div>Floor (P10): <strong>{shield?.p10_xp ?? "—"} xP</strong></div>
              <div>Ceiling (P90): <strong>{shield?.p90_xp ?? "—"} xP</strong></div>
            </div>
          </div>
        </div>

        {/* DIAMOND */}
        <div className="bg-surface border border-secondary/30 rounded-3xl p-6 relative overflow-hidden shadow-xl space-y-4">
          <div className="flex items-center justify-between">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-secondary/15 text-secondary text-xs font-bold uppercase tracking-wider">
          ...
          <div>
            <h3 className="text-2xl font-black text-slate-100">{diamond?.web_name ?? "No eligible pick"}</h3>
            <div className="text-xs text-slate-400 font-medium">Strongest Overall Expected Score</div>
          </div>

          <div className="bg-surfaceHover/60 border border-surfaceBorder rounded-2xl p-4 flex items-baseline justify-between font-mono">
            <div>
              <div className="text-2xl font-extrabold text-secondary">{diamond?.mean_xp ?? "—"} xP</div>
              <div className="text-[10px] text-slate-400 font-sans">Mean Forecast</div>
            </div>
            <div className="text-right text-xs text-slate-300 font-sans">
              <div>Floor (P10): <strong>{diamond?.p10_xp ?? "—"} xP</strong></div>
              <div>Ceiling (P90): <strong>{diamond?.p90_xp ?? "—"} xP</strong></div>
            </div>
          </div>
        </div>

        {/* SWORD */}
        <div className="bg-surface border border-rose-400/25 rounded-3xl p-6 relative overflow-hidden shadow-xl space-y-4">
          <div className="flex items-center justify-between">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-400/10 text-rose-300 text-xs font-bold uppercase tracking-wider">
          ...
          <div>
            <h3 className="text-2xl font-black text-slate-100">{sword?.web_name ?? "No eligible pick"}</h3>
            <div className="text-xs text-slate-400 font-medium">Low Ownership • Explosive Ceiling</div>
          </div>

          <div className="bg-surfaceHover/60 border border-surfaceBorder rounded-2xl p-4 flex items-baseline justify-between font-mono">
            <div>
              <div className="text-2xl font-extrabold text-rose-300">{sword?.mean_xp ?? "—"} xP</div>
              <div className="text-[10px] text-slate-400 font-sans">Mean Forecast</div>
            </div>
            <div className="text-right text-xs text-slate-300 font-sans">
              <div>Floor (P10): <strong>{sword?.p10_xp ?? "—"} xP</strong></div>
              <div>Ceiling (P90): <strong>{sword?.p90_xp ?? "—"} xP</strong></div>
            </div>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-surfaceBorder text-slate-200 font-mono">
              {ranked.map((c: any, idx: number) => (
                <tr key={c.player_id} className="hover:bg-surfaceHover/50 transition-colors">
                  ...
                  <td className="py-3.5 px-4 text-rose-300 font-bold">{c.p90_xp} xP</td>
